...
print('------------------------------------------------------')
dfPerfil.rename(columns=  {'ESTADO CIVIL':'ESTCIV', 'ESCOLARIDADE COMPLETA': 'ESCOLA', 'RENDIMENTO MENSAL': 'RENDM'}, inplace=True)
print(dfPerfil)
...
dfPerfil.fillna(value = {"IDADE" : agEstCiv.IDADE.transform(min)}, inplace = True)
print(dfPerfil)

print('------------------------------------------------------')
print('1.e- Rendimento preenchido')
print('------------------------------------------------------')
dfPerfil.fillna(value= {'RENDM':dfPerfil.RENDM.mean()},inplace=True)
print(dfPerfil)
...
print('------------------------------------------------------')
dfPerfil.RENDM = pd.cut(dfPerfil.RENDM, bins = 4, labels = ["D", "C", "B", "A"])
srTabFreqRend = dfPerfil.RENDM.value_counts()
srTabFreqRend.plot.pie(title = "Tab Freq Rendimentos", autopct = "%.1f")
plt.show()

print('------------------------------------------------------')
print('2.b- Tabela de frequ�ncia de categoria de idade x UF')
print('------------------------------------------------------')
dfPerfil.CATID = pd.cut(dfPerfil.IDADE, bins = [0, 30, 50, 70, dfPerfil.IDADE.max()], labels = ["JOVEM", "ADULTO", "SENIOR", "AVANCADDO"])
ctCatIdxUF = pd.crosstab(index = dfPerfil.IDADE, columns = dfPerfil.UF)
print(ctCatIdxUF)
...
print('------------------------------------------------------')
srPercGen = dfPerfil.GENERO.value_counts()
print(srPercGen)
print((srPercGen / dfPerfil.GENERO.size) * 100)

print('------------------------------------------------------')
print('3.b- Gr�fico de Barras das idades dos que n�o s�o de RJ')
print('------------------------------------------------------')
dfNaoRio = dfPerfil.loc[dfPerfil.UF != "RJ"]
dfNaoRio.IDADE.plot.bar(title = "Idades fora do Rio")
plt.show()
